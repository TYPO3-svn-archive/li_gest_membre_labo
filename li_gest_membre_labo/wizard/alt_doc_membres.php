...
/**
**********      ATTENTION *************
* Ce fichier est une personnalisation du fichier alt_doc.php adaptée à l'extension de gestion de membres
**************************************** */
/**
 ...
 *
 * $Id: alt_doc.php 4256 2008-09-28 19:52:42Z ohader $
...
class SC_alt_doc_membres {
	...
	var $retUrl;			// Return URL script, processed. This contains the script (if any) that we should RETURN TO from the alt_doc_membres.php script IF we press the close button. Thus this variable is normally passed along from the calling script so we can properly return if needed.
	...
	var $idMembreLabo; //Identifiant du membre du labo à activer
	...
	function preInit()	{
		...
		if (!is_array($this->defVals) && is_array($this->overrideVals))	{
			...
			// Setting return URL
		$this->retUrl = $this->returnUrl ? $this->returnUrl : $this->BACK_PATH.'dummy.php';
		...
		$this->docDat = $BE_USER->getModuleData('alt_doc_membres.php','ses');
	...
	function init()	{
		...
		$this->idMembreLabo = 0;
		$this->idMembreLabo = intval(t3lib_div::_GP('idMembreLabo'));
		...
		$this->MCONF['name']='xMOD_alt_doc_membres.php';
		...
		if (is_array($this->editconf))	{
			...
			if ($editForm)	{
				...
				if ((strcmp($this->docDat[1], $this->storeUrlMd5) || !isset($this->docHandler[$this->storeUrlMd5])) && !$this->dontStoreDocumentRef) {
						...
						$BE_USER->pushModuleData('alt_doc_membres.php', array($this->docHandler, $this->storeUrlMd5));
						t3lib_BEfunc::setUpdateSignal('tx_opendocs::updateNumber', count($this->docHandler));
				}


					// Module configuration
				$this->modTSconfig = ($this->viewId ? t3lib_BEfunc::getModTSconfig($this->viewId,'mod.xMOD_alt_doc_membres') : array());
						...
						foreach($ids as $theUid)	{
							...
							if ($hasAccess)	{
								...
								if (is_array($rec))	{
									...
									// S'il y a un identifiant de membre indiqué, on le sélectionne par défaut
									// Cette partie de code permet de rechercher dans la page HTML qui sera affichée la zone qui concerne l'identifiant du membre pour indiquer celui que l'on veut par défaut
									if($this->idMembreLabo>0){

										$texte = $panel;
										// Définition du masque de la zone d'affichage que l'on recherche
										$pattern = "#<td valign=[\"']top[\"']>(.*)</select></td>#Uis";

										// On récupère dans $soustexte les zones de texte comprisent à l'intérieur du masque
										preg_match_all($pattern , $texte, $souszone, PREG_PATTERN_ORDER);
										

										foreach($souszone[0] as $cettesouszone)
										{
											//Pour chacune des zones, on recherche celles qui concerne le membre (qui contient idMembreLabo)

											if (preg_match("#idMembreLabo#", $cettesouszone))
											{
												// On indique dans la sous-zone l'identifiant par défaut
												$cettesouszonenouvelle = str_replace('value="'.$this->idMembreLabo.'"','value="'.$this->idMembreLabo.'" selected="selected"',$cettesouszone);
												
												// On met ensuite à jour la page complête en remplaçant la sous-zone par la nouvelle
												$panel = str_replace($cettesouszone,$cettesouszonenouvelle,$panel);
											}
	...
	protected function getButtons()	{
		...
		if (!$this->errorC && !$TCA[$this->firstEl['table']]['ctrl']['readOnly'])	{
			...
			// CLOSE button:
		$buttons['close'] = '<a href="#" onclick="document.editform.closeDoc.value=1; document.editform.submit(); window.opener.location.reload(); return false; ">'.
		...
		if (!$this->errorC && !$TCA[$this->firstEl['table']]['ctrl']['readOnly'] && count($this->elementsData)==1)	{
			if ($this->firstEl['cmd']!='new' && t3lib_div::testInt($this->firstEl['uid']))	{
				...
				if ($this->getNewIconMode($this->firstEl['table'],'showHistory'))	{
					$aOnClick = 'window.location.href='.$this->BACK_PATH.'show_rechis.php?element='.rawurlencode($this->firstEl['table'].':'.$this->firstEl['uid']).'&returnUrl='.rawurlencode($this->R_URI).'\'; return false;';
	...
	function functionMenus()	{
		...
		return '
				...
				<br />'.t3lib_BEfunc::getFuncCheck('','SET[showPalettes]',$this->MOD_SETTINGS['showPalettes'],'alt_doc_membres.php',t3lib_div::implodeArrayForUrl('',array_merge($this->R_URL_getvars,array('SET'=>''))),'id="checkShowPalettes"').'<label for="checkShowPalettes">'.$LANG->sL('LLL:EXT:lang/locallang_core.php:labels.showPalettes',1).'</label>';
	}
						...
						if ($GLOBALS['BE_USER']->checkLanguageAccess($lang['uid']))	{
							...
							if($newTranslation) {
								$href = $this->doc->issueCommand(
									...
									$this->backPath.'alt_doc_membres.php?justLocalized='.rawurlencode($table.':'.$rowsByLang[0]['uid'].':'.$lang['uid']).'&returnUrl='.rawurlencode($this->retUrl)
								);

								// create edit url
							} else {
								$href = $this->backPath.'alt_doc_membres.php?';
	...
	/**
	 * Redirects to alt_doc_membres with new parameters to edit a just created localized record
	 *
	...
	function localizationRedirect($justLocalized)	{
		...
		if ($TCA[$table] && $TCA[$table]['ctrl']['languageField'] && $TCA[$table]['ctrl']['transOrigPointerField'])	{
			...
			if (is_array($localizedRecord))	{
				...
				$location = $GLOBALS['BACK_PATH'].'alt_doc_membres.php?'.$params.$returnUrl;
	...
	function closeDocument($code=0)	{
		...
		if (isset($this->docHandler[$this->storeUrlMd5])) {
			...
			$BE_USER->pushModuleData('alt_doc_membres.php', array($this->docHandler, $this->docDat[1]));
			t3lib_BEfunc::setUpdateSignal('tx_opendocs::updateNumber', count($this->docHandler));
		}
...
// Include extension?
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/li_gest_membre_labo/wizard/alt_doc_membres.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/li_gest_membre_labo/wizard/alt_doc_membres.php']);
}
...
// Make instance:
$SOBE = t3lib_div::makeInstance('SC_alt_doc_membres');
